...
with tab3:
    ...
    if population_df.empty:
        ...
        # 清理欄位名稱與區域別
        # -----------------------------
        population_df.columns = [c.strip().replace("　", "") for c in population_df.columns]
        population_df["區域別"] = population_df["區域別"].str.strip().replace("　", "")
        ...
        # 將人口欄位全部轉整數
        for col in population_df.columns[1:-2]:  # 排除縣市、行政區欄位
        ...
        pop_long["年份"] = pd.to_numeric(pop_long["年份"].astype(str).str.strip(), errors="coerce").fillna(0).astype(int)
    ...
    with col1:
        ...
        # -----------------------------
            ...
            if population_df.empty or filtered_df.empty:
                ...
                pop_latest = population_df[["區域別", sorted(population_df.columns[1:-2])[-1]]].copy()
