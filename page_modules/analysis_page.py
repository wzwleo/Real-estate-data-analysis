        ...
        with col1:
            if st.session_state.show_filtered_data:
                ...
                if chart_type == "不動產價格趨勢分析" and len(filtered_df) > 0:
                    ...
                    new_data = [
                        int(yearly_avg[(yearly_avg['年份']==y) & (yearly_avg['BUILD']=='新成屋')]['平均單價元平方公尺'].mean())
                        for y in years
                    ]
                    old_data = [
                        int(yearly_avg[(yearly_avg['年份']==y) & (yearly_avg['BUILD']=='中古屋')]['平均單價元平方公尺'].mean())
                        for y in years
                    ]
                    option = {
                    ...
                    df_group = filtered_df.groupby(['年份', 'BUILD'])['交易筆數'].sum().reset_index()
                    years = sorted(df_group['年份'].unique())
                    year_labels = [str(y) for y in years]

                    new_data = [
                        int(df_group[(df_group['年份']==y) & (df_group['BUILD']=='新成屋')]['交易筆數'].sum())
                        for y in years
                    ]
                    old_data = [
                        int(df_group[(df_group['年份']==y) & (df_group['BUILD']=='中古屋')]['交易筆數'].sum())
                        for y in years
                    ]
                    option = {
                        "tooltip": {"trigger": "axis"},
                        ...
                        "yAxis": {"type": "value"},
                        "series": [
                            {"name": "新成屋", "type": "bar", "data": new_data},
                            {"name": "中古屋", "type": "bar", "data": old_data}
                        ]
                    }
                    st_echarts(option, height="400px")
