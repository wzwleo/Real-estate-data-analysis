...
"""
        ...
        with tab3:
            ...
            if population_df.empty:
                ...
                # 解析民國年為西元
                def parse_year_season(col_name):
                    try:
                        year = int(col_name.split("年")[0]) + 1911
                        return year
                    except:
                        return 0
        
                pop_long["年份"] = pop_long["年度季度"].apply(parse_year_season)
                pop_long = pop_long.sort_values(["縣市", "行政區", "年份"])
            ...
            # -----------------------------
            st.markdown("## 👥 人口統計")
            st.dataframe(filtered_population, use_container_width=True)
            ...
            # 圖表邏輯
            # -----------------------------
            if chart_type == "不動產價格趨勢分析" and len(filtered_realestate) > 0:
                filtered_realestate["年份"] = filtered_realestate["季度"].str[:3].astype(int) + 1911
                yearly_avg = filtered_realestate.groupby(["年份", "BUILD"])["平均單價元平方公尺"].mean().reset_index()
                ...
                if city_choice == "全台":
                    ...
                    trans_counts = filtered_realestate.groupby("行政區")["交易筆數"].sum().reset_index()
                ...
                option = {
                    ...
                    "series": [
                        {"name": "人口數", "type": "line", "data": merged["人口數"].tolist(), "smooth": True},
                ...
                latest_year_col = sorted(pop_cols)[-1]
                pop_latest = filtered_population[filtered_population["年度季度"] == latest_year_col][["縣市","行政區","人口數"]]
                price_avg = filtered_realestate.groupby(["縣市", "行政區"])["平均單價元平方公尺"].mean().reset_index()
                merged = pd.merge(pop_latest, price_avg, on=["縣市","行政區"], how="inner")
