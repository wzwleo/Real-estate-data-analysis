...
CHINESE_TO_GOOGLE_TYPE = {}  # 中文關鍵字到 Google Places API 類型的映射
GOOGLE_TYPE_TO_CHINESE = {}  # Google Places API 類型到中文關鍵字的映射（用於反向查找）

for category, items in PLACE_TYPES.items():
    for i in range(0, len(items), 2):
        if i+1 < len(items):
            chinese_name = items[i]
            google_type = items[i+1]
            
            CHINESE_TO_CATEGORY[chinese_name] = category
            CHINESE_TO_GOOGLE_TYPE[chinese_name] = google_type
            
            # 建立反向映射，但處理一對多的情況
            if google_type not in GOOGLE_TYPE_TO_CHINESE:
                GOOGLE_TYPE_TO_CHINESE[google_type] = []
            GOOGLE_TYPE_TO_CHINESE[google_type].append(chinese_name)

# 建立從 Google 類型到中文關鍵字的單一映射（選擇第一個）
GOOGLE_TYPE_TO_CHINESE_SIMPLE = {}
for google_type, chinese_list in GOOGLE_TYPE_TO_CHINESE.items():
    if chinese_list:
        GOOGLE_TYPE_TO_CHINESE_SIMPLE[google_type] = chinese_list[0]
