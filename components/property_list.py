...
def render_property_list():
    ...
    current_page_data, _, total_pages, total_items = display_pagination(df, items_per_page=10, page=current_page)
...
def render_pagination_controls(current_page, total_pages, total_items):
    ...
    with col1:
        if st.button("⏮️ 第一頁", key="first_page", disabled=(current_page == 1)):
            st.session_state.current_search_page = 1

    with col2:
        if st.button("⏪ 上一頁", key="prev_page", disabled=(current_page == 1)):
    ...
    with col4:
        if st.button("下一頁 ⏩", key="next_page", disabled=(current_page == total_pages)):
            st.session_state.current_search_page = min(total_pages, current_page + 1)

    with col5:
        if st.button("最後一頁 ⏭️", key="last_page", disabled=(current_page == total_pages)):
