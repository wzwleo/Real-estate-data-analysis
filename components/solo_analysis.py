    ...
    if analyze_clicked:
        ...
        try:
            ...
            with st.spinner("正在將資料進行向量化處理..."):
                def row_to_text(row):
                    return (f"地址:{row['地址']}, 建坪:{row['建坪']}, 主+陽:{row['主+陽']}, "
                ...
                selected_text_display = f"{selected_row['標題']} - {row_to_text(selected_row)}"
                relevant_text = "\n".join([f"{r['標題']} - {row_to_text(r)}" for r in relevant_data])

                prompt = f"""
                你是一位台灣不動產市場專家，具有多年房屋估價與市場分析經驗。
                請根據以下房屋資料生成中文市場分析：
                目標房型： {selected_text_display}
                相似房屋資料： {relevant_text}
                ...
                【目標房型資料】 {selected_text_display}
                【相似房屋資料】 {relevant_text}
                請根據以上資訊，對六項指標各自評分 0～10 分。
                以純 JSON 回覆，不要加入任何解釋文字。
                請直接回傳純 JSON 格式，**不要添加任何說明、文字或換行**
                JSON 格式如下：
