        ...
        if analyze_clicked:
            ...
            try:
                with st.spinner("📊 正在計算市場價格指標..."):
                    ...
                    age_prompt = f"""
                    以下是「已計算完成」的屋齡分析數據（JSON）。
                    請 **僅依據提供的數值說明**，不得自行推論、延伸或補充未出現的資料。
                    
                    請用繁體中文，並 **限制在 6 句以內、每句不超過 20 字**，完成以下三點：
                    1️⃣ 屋齡整體評價（{age_analysis_payload['屋齡分布']['屋齡評估']}）
                    2️⃣ 屋齡對價格與維護成本的影響（各 1 句）
                    3️⃣ 購屋建議與注意事項（1–2 句）
