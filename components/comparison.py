...
class ComparisonAnalyzer:
    ...
    def _generate_single_analysis_zip(self, name, analysis):
        ...
        with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zip_file:
            ...
            if not df.empty:
                ...
                zip_file.writestr(
                    f"{safe_name}_設施清單_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx",
            ...
            safe_name = re.sub(r'[\\/*?:"<>|]', "", name)[:30]
            zip_file.writestr(
                f"{safe_name}_分析報告_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt",
    ...
    def _generate_single_txt_report(self, name, analysis):
        ...
        if not df.empty:
            ...
            for idx, row in df_sorted.iterrows():
                ...
                txt_lines.append(
                    f"  • {row['設施名稱']} ({row['設施子類別']}){is_nuisance} - {row['距離(公尺)']}公尺"
                )
            txt_lines.append("")
    ...
    def render_comparison_tab(self):
        ...
        try:
                            ...
                            if st.button("📥", key=f"download_{name}", help="下載此分析"):
                                with st.spinner("生成報告中..."):
                                    ...
                                    if zip_data:
                                        ...
                                        href = (f'<a href="data:application/zip;base64,{b64}" '
                                                f'download="{filename}">✅ 點此下載</a>')
            ...
            if (st.session_state.current_analysis_name and
                    st.session_state.current_analysis_name in st.session_state.saved_analyses):
...
background-color:{bg}; text-align:center; height:170px; margin-bottom:10px;">
...
""", unsafe_allow_html=True)
        ...
        if current_profile:
            ...
            st.success(
                f"✅ 當前選擇：**{profile_info['icon']} {current_profile}** | "
                f"📌 分析重點：{profile_info['prompt_focus'][0]}、{profile_info['prompt_focus'][1]}..."
            )
        else:
        ...
        radius = st.slider(
            f"搜尋半徑（{profiles[current_profile]['icon']} 建議：{suggest_r}公尺）",
...
<span style="background-color:{rec_color}; color:black; padding:2px 6px;
border-radius:12px; font-size:10px; margin-left:5px;">{rec_text}</span>
</div>
""", unsafe_allow_html=True)
                                    cb = st.checkbox(" ", key=f"sub_{cat}_{idx}",
                                                     label_visibility="collapsed", value=default_val)
                                else:
            ...
            with st.status("🔍 分析進行中...", expanded=True) as status:
                ...
                for idx, (name, info) in enumerate(houses_data.items()):
                    ...
                    nuisances = self._query_nuisances_no_progress(
                        info["lat"], info["lng"], s["server"], s["nuisances"], s["radius"]
                    )
...
""", unsafe_allow_html=True)
        ...
        with c1:
            if '總價元' in house:
                st.metric("總價", f"{int(house['總價元']):,} 元")
        with c2:
            if '建物面積平方公尺' in house:
                st.metric("面積", f"{house['建物面積平方公尺']:.1f} ㎡")
        with c3:
            if '平均單價元平方公尺' in house:
                st.metric("單價", f"{int(house['平均單價元平方公尺']):,} 元/㎡")
    ...
    def _start_analysis(self, mode, houses, radius, keyword, cats, subs, fav_df, profile,
                        include_nuisance=False, selected_nuisances=None):
        """開始分析 - 四種情境"""
        try:
            st.session_state.analysis_settings = {
                "mode": mode,
    ...
    def _display_analysis_results(self, res):
        ...
        if mode == "單一房屋分析":
            st.markdown(
                f"## {icon} {profile}視角 · 單一房屋分析" + (" (含嫌惡設施)" if include_nuisance else "")
            )
        else:
            st.markdown(
                f"## {icon} {profile}視角 · {res['num_houses']}間房屋比較" + (" (含嫌惡設施)" if include_nuisance else "")
            )
                ...
                if option_str:
                    ...
                    if house_id:
                        ...
                        if depth_item:
                            ...
                            depth_data[house_name] = {'scores': scores, 'summary': summary, 'has_data': True}
                        else:
                            depth_data[house_name] = {'has_data': False, 'summary': '尚未進行深度分析'}
                    else:
                        depth_data[house_name] = {'has_data': False, 'summary': '無法比對房屋編號'}
                else:
                    depth_data[house_name] = {'has_data': False, 'summary': '無法取得房屋資訊'}
        ...
        if not df.empty:
            ...
            st.info(f"📈 共找到 {len(df)} 筆資料" +
                    (f"（包含 {nuisance_count} 處嫌惡設施）" if nuisance_count > 0 else ""))
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "place_id": st.column_config.TextColumn(
                        "Google地圖", help="點擊連結在Google地圖中查看", width="small"
                    )
                },
    ...
    def _display_maps(self, res):
        ...
        if len(houses) == 1:
            ...
            self._render_map_with_links(houses[n]["lat"], houses[n]["lng"], places[n], radius, n, houses[n], bk)
        elif len(houses) <= 3:
            cols = st.columns(len(houses))
            for i, (n, info) in enumerate(houses.items()):
                with cols[i]:
                    st.markdown(f"### {n}")
                    self._render_map_with_links(info["lat"], info["lng"], places[n], radius, n, info, bk)
        else:
    ...
    def _render_map_with_links(self, lat, lng, places, radius, title, house_info, browser_key):
        ...
        facilities_data = []
        for p in places:
            color = "#dc3545" if p[0] == "嫌惡設施" else CATEGORY_COLORS.get(p[0], "#666")
            facilities_data.append({
                "name": p[2], "category": p[0], "subtype": p[1],
                "lat": p[3], "lng": p[4], "distance": p[5], "color": color,
                "place_id": p[6],
                "maps_url": f"https://www.google.com/maps/search/?api=1&query={p[3]},{p[4]}&query_place_id={p[6]}"
            })

        categories = {f["category"]: f["color"] for f in facilities_data}
        legend_html = ""
        for cat, color in categories.items():
            legend_html += f'<div class="legend-item"><div class="legend-color" style="background-color:{color};"></div><span>{cat}</span></div>\n'
...
#legend {{
  ...
  font-size: 12px; margin: 10px; max-width: 200px;
  box-shadow: 0 2px 6px rgba(0,0,0,0.1); position: absolute; right: 10px; top: 10px; z-index: 1000;
}}
...
function initMap() {{
  ...
  var mainInfoContent = '<div class="info-window"><h5>🏠 {title}</h5>' +
    '<p><strong>地址：</strong>{address_str}</p>' +
    '<p><strong>搜尋半徑：</strong>{radius} 公尺</p>' +
    '<p><strong>設施數量：</strong>{len(places)} 個</p></div>';
  ...
  facilities.forEach(function(facility) {{
    ...
    var marker = new google.maps.Marker({{
      ...
      icon: {{
        ...
        fillColor: facility.color, fillOpacity: 0.9, strokeColor: "#FFFFFF", strokeWeight: 2
      }},
      animation: google.maps.Animation.DROP
    }});
    var infoContent = '<div class="info-window"><h5>' + facility.name + '</h5>' +
      '<p><span style="color:' + facility.color + '; font-weight:bold;">' +
      facility.category + ' - ' + facility.subtype + '</span></p>' +
      '<p><strong>距離：</strong>' + facility.distance + ' 公尺</p>' +
      '<a href="' + facility.maps_url + '" target="_blank" class="maps-link">🗺️ 在 Google 地圖中查看</a></div>';
...
function handleMapError() {{
  document.getElementById('map').innerHTML =
    ...
    '<h3>❌ 地圖載入失敗</h3><p>請檢查 Google Maps API Key 是否正確</p></div>';
}}
</script>
<script src="https://maps.googleapis.com/maps/api/js?key={browser_key}&callback=initMap"
  async defer onerror="handleMapError()"></script>
                    ...
                    for i, p in enumerate(places, 1):
                        ...
                        with col2:
                            st.markdown(
                                ...
                                f'padding:4px 8px; border-radius:8px; font-size:12px; font-weight:bold;">'
                                f'{subtype}</span>',
                                unsafe_allow_html=True
                            )
                        with col3:
                            st.markdown(
                                f'<span style="background-color:{dist_color}20; color:{dist_color}; '
                                f'padding:4px 8px; border-radius:8px; font-size:12px; font-weight:bold;">'
                                f'{dist}公尺 ({dist_badge})</span>',
                                unsafe_allow_html=True
                            )
                        with col4:
    ...
    def _display_ai_analysis(self, res):
        ...
        if mode == "單一房屋分析":
            prompt = (self._build_single_with_nuisance_prompt(res, facilities_text, profile, icon, pinfo)
                      if include_nuisance
                      else self._build_single_without_nuisance_prompt(res, facilities_text, profile, icon, pinfo))
        else:
            prompt = (self._build_multi_with_nuisance_prompt(res, facilities_text, profile, icon, pinfo)
                      if include_nuisance
                      else self._build_multi_without_nuisance_prompt(res, facilities_text, profile, icon, pinfo))
        ...
        with c1:
            edited = st.text_area("📝 AI 分析提示詞設定（可編輯）", st.session_state.custom_prompt,
                                  height=350, key="pedit")
        ...
        with c2:
            ...
            st.markdown("---")
            st.markdown("**提示：** 您可以編輯左側的提示詞，讓AI更符合您的需求")
        ...
        if "gemini_result" in st.session_state:
            ...
            if (st.session_state.current_analysis_name and
                    st.session_state.current_analysis_name in st.session_state.saved_analyses):
                st.session_state.saved_analyses[st.session_state.current_analysis_name]['gemini_result'] = \
            ...
            with c2:
                ...
                if mode != "單一房屋分析":
                    report_title = (f"{profile}視角-{res['num_houses']}間房屋"
                                    f"{'含嫌惡設施' if include_nuisance else '生活機能'}比較報告")
    ...
    def _build_single_without_nuisance_prompt(self, res, facilities_text, profile, icon, pinfo):
        ...
        return f"""你是一位專業的房地產分析師，請以「{icon} {profile}」的身份，對以下房屋進行生活機能分析與評分。

【買家類型】{profile} - {pinfo.get('description', '')}
重點關注：{', '.join(focus)}
...
1. **綜合評分**（1-10分）
- 交通便利性（滿分3分）- 日常採買（滿分3分）- 生活品質（滿分2分）- 特殊需求（滿分2分）

2. **主要優點**（3-5點）引用具體設施名稱和距離。

3. **主要缺點**（3-5點）指出缺少哪些重要設施。

4. **生活便利性預測**（平日、假日、緊急情況）

5. **未來發展潛力**（3-5年）

6. **購買建議**

請用專業、客觀角度分析，引用具體設施名稱和距離。"""
    ...
    def _build_single_with_nuisance_prompt(self, res, facilities_text, profile, icon, pinfo):
        ...
        cnt = res["facility_counts"].get(name, 0)
        focus = pinfo.get("prompt_focus", [])
        nuisance_count = len(res['nuisance_data'].get(name, [])) if res.get('nuisance_data') else 0

        return f"""你是一位專業的房地產分析師，請以「{icon} {profile}」的身份分析以下房屋。**此分析包含嫌惡設施評估**。
...
【搜尋條件】半徑：{res['radius']} 公尺 | 分析類別：{', '.join(res['selected_categories'])}

【周邊設施統計】總數量：{cnt} 個（含 {nuisance_count} 處嫌惡設施）
...
1. **綜合評分**（1-10分）含嫌惡設施扣分（最高-3分）

2. **主要優點**（3-5點）

3. **主要缺點**（3-5點）**必須包含嫌惡設施的影響**（類型、距離、潛在影響、對房價影響）

4. **生活便利性預測**（考慮嫌惡設施）

5. **未來發展潛力**（評估嫌惡設施是否可能改善）
...
請用專業、客觀角度分析。"""
    ...
    def _build_multi_without_nuisance_prompt(self, res, facilities_text, profile, icon, pinfo):
        ...
        comparison_text = "\n".join([f"  {name}：{cnt} 個設施" for name, cnt in counts.items()])

        return f"""你是一位專業的房地產分析師，請比較以下{len(houses_data)}間房屋，以「{icon} {profile}」身份給出建議。
...
1. **綜合排名**（1-{len(houses_data)}名）詳細說明排名依據。

2. **各房屋評分**（1-10分）含交通便利性(3)、日常採買(3)、生活品質(2)、特殊需求(2)。

3. **優缺點比較表**（交通便利性、日常採買、生活品質、價格效益、未來潛力）

4. **各房屋詳細分析**（優勢、劣勢、適合{profile}的程度）
...
請引用具體設施名稱和距離。"""
    ...
    def _build_multi_with_nuisance_prompt(self, res, facilities_text, profile, icon, pinfo):
        ...
        return f"""你是一位專業的房地產分析師，請比較以下{len(houses_data)}間房屋，以「{icon} {profile}」身份給出建議。**此分析包含嫌惡設施評估**。
...
2. **各房屋評分**（1-10分）含嫌惡設施扣分（最高-3分）

3. **優缺點比較表**（含嫌惡設施影響欄）

4. **各房屋詳細分析**（優勢、劣勢含嫌惡設施、適合{profile}程度）

5. **最終推薦**（考慮嫌惡設施影響）

6. **購買時機建議**（考慮嫌惡設施對房價長期影響）
    ...
    def _format_all_facilities_for_prompt(self, res):
        ...
        for house_name in df['房屋'].unique():
            ...
            for cat, count in cat_summary.items():
                is_nuisance_cat = " ⚠️" if cat in NUISANCE_TYPES.keys() else ""
