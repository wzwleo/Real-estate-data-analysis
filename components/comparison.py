import streamlit as stimport pandas as pdimport timeimport jsonimport sysimport osimport requestsimport mathfrom streamlit.components.v1 import htmlfrom streamlit_echarts import st_echartsfrom collections import Counterimport base64from datetime import datetime, timedeltaimport ioimport reimport zipfileimport pytz

修正匯入路徑

current_dir = os.path.dirname(os.path.abspath(file))parent_dir = os.path.dirname(current_dir)if parent_dir not in sys.path:sys.path.insert(0, parent_dir)

try:from config import CATEGORY_COLORS, DEFAULT_RADIUSfrom components.place_types import PLACE_TYPES, CHINESE_TO_CATEGORY, NUISANCE_TYPES, IMPACT_TYPESfrom components.geocoding import geocode_address, haversineCONFIG_LOADED = Trueexcept ImportError as e:CONFIG_LOADED = Falsest.warning(f"無法載入設定: {e}")PLACE_TYPES = {}NUISANCE_TYPES = {}CHINESE_TO_CATEGORY = {}CATEGORY_COLORS = {}DEFAULT_RADIUS = 500IMPACT_TYPES = {}

設定台灣時區

try:TZ_TAIWAN = pytz.timezone('Asia/Taipei')except:TZ_TAIWAN = None

def get_taiwan_time():"""取得台灣時間"""if TZ_TAIWAN:return datetime.now(TZ_TAIWAN).strftime('%Y-%m-%d %H:%M:%S')else:utc_now = datetime.utcnow()taiwan_time = utc_now + timedelta(hours=8)return taiwan_time.strftime('%Y-%m-%d %H:%M:%S')

class ComparisonAnalyzer:"""房屋分析器 - 支援單一分析和多房屋比較"""
...
【🏠 房屋深度分析】💰 價格：{price}萬（低於{price_pct:.0f}%的房屋，比市場中位數{price_direction}{abs(vs_median)}萬）📐 空間：使用率{usage:.0%}（高於{usage_pct:.0f}%的房屋）🕰 屋齡：{age}年（{age_cat}，比{100-age_pct:.0f}%的房屋新）🏢 樓層：{floor}樓（{floor_cat}，比{floor_pct:.0f}%的房屋{"低" if floor_pct < 50 else "高"}）🛋 格局：{layout}（市場排名第{layout_rank}，佔{layout_pct:.1f}%）"""except Exception as e:return ""
...
【買家類型】{profile} - {pinfo.get('description', '')}重點關注：{', '.join(focus)}{depth_text}【房屋資訊】
...
{facilities_text}{nuisance_text}

請根據【房屋深度分析】、【一般設施清單】和【嫌惡設施清單】綜合評估，提供以下分析：

整體居住適合度：不用分數，改用「適合 / 可考慮 / 需謹慎」描述。

主要優點（3-5點）

主要缺點（3-5點）：必須說明最近嫌惡設施、距離與可能影響。

生活便利性判斷

嫌惡設施提醒：列出最需要現場確認的項目。

購買建議：給出是否值得看屋、是否需要議價或避開的建議。
...
【買家類型】{profile} - {pinfo.get('description', '')}重點關注：{', '.join(focus)}{depth_section}【候選房屋】{house_list}

【各房屋設施數量】{comparison_text}

{facilities_text}

請根據【各房屋深度分析】和【一般設施清單】綜合評估，提供以下分析：

綜合排名（1-{len(houses_data)}名）
...
綜合排名：不用嫌惡設施分數，改用距離、數量、類型、影響分類與買家需求判斷。

各房屋優缺點比較表

嫌惡設施比較：列出每間房屋最近的嫌惡設施、距離與數量。
...
def get_comparison_analyzer():return ComparisonAnalyzer()
